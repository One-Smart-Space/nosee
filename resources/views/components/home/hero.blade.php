...
<section
    ...
    data-hero-carousel
>
    {{-- Slide only the images; the content panels below remain fixed in place. --}}
            ...
            >
                <img
                    ...
                    @else
                        loading="lazy"
                    @endif
                    class="absolute top-0 left-1/2 h-[600px] w-[968px] max-w-none -translate-x-1/2 object-cover object-center lg:inset-0 lg:h-full lg:w-full lg:translate-x-0"
                >
            </div>
        @endforeach
    </div>
    ...
    <x-ui.container class="relative h-full">
        @foreach ($items as $item)
            ...
            >
                <div class="absolute top-1/2 right-4 left-4 flex -translate-y-1/2 flex-col items-start gap-5 lg:top-[159px] lg:right-auto lg:left-[120px] lg:w-[852px] lg:translate-y-0 lg:gap-0">
                    <p class="rounded-[2px] bg-white/40 px-2.5 py-1.5 text-xs leading-4 text-primary uppercase lg:rounded-none lg:bg-default lg:px-3 lg:py-1 lg:text-sm lg:leading-6">
                        {{ $item['eyebrow'] }}
                    </p>

                    <div class="flex w-full flex-col gap-3 text-inverse lg:mt-1 lg:gap-4">
                        <h1 class="font-heading text-[28px] leading-8 font-semibold lg:text-[56px] lg:leading-[64px]">
                            {{ $item['title'] }}
                        </h1>

                        <p class="max-w-[525px] text-base lg:text-xl">
                            {{ $item['summary'] }}
                        </p>
                    </div>
                    ...
                    <x-ui.button
                        ...
                        size="lg"
                        icon-position="right"
                        class="gap-10 px-6 tracking-[0.06em] uppercase lg:mt-8"
                    >
                        {{ $item['cta_label'] }}
                        ...
                        <x-slot:icon>
                            <span class="relative block size-4">
                                <img
                                    src="/media/icons/arrow-right-line.svg"
                                    alt=""
                                    width="13"
                                    height="2"
                                    class="absolute top-[7.25px] left-[1.75px] h-[1.5px] w-[12.5px] max-w-none"
                                >
                                <img
                                    src="/media/icons/arrow-right-head.svg"
                                    alt=""
                                    width="7"
                                    height="11"
                                    class="absolute top-[2.5px] right-[1.5px] h-[11px] w-[6.5px] max-w-none"
                                >
                            </span>
                        </x-slot:icon>
                    </x-ui.button>
                ...
                <p class="absolute right-[120px] bottom-[75px] hidden max-w-[294px] text-right text-sm leading-[18px] lg:block">
                    {{ $item['image_credit'] }}
                </p>
            </article>
        @endforeach
    ...
    @if ($itemCount > 1)
        <x-ui.container class="pointer-events-none absolute inset-x-0 bottom-4 lg:bottom-8" data-hero-controls>
            <div class="pointer-events-auto flex gap-2 lg:ml-[88px] xl:ml-14" aria-label="Hero slides">
                @foreach ($items as $item)
                    <button
                        type="button"
                        @class([
